...
// Forward-declarations only — libvterm headers stay out of this header.
struct VTerm;
struct VTermScreen;
...
public:
    ...
    // Keyboard input. libvterm generates the appropriate escape sequence
    // and writes it to the internal output buffer; drain via take_output()
    ...
    // --- Scrollback ---------------------------------------------------------
    // Lines that have scrolled off the top, kept so callers can present a
    // scroll-up view. Index 0 is the oldest saved line; index
    // scrollback_lines()-1 is the most recent (immediately above the live
    // screen).
